...
router.post(`/registration`,[
 check('email',"Incorrect email").isEmail(),
    check('password','Password must be longer then 3 and shorter than 12').isLength({min:3,max:12}),
        check('name','Name must be longer then 3 and shorter than 10').isLength({min:3,max:10})
],
    async(req,res)=>{
    try{
...
router.post('/login',
    async(req,res)=>{
        ...
        const isPassValid =bcrypt.compareSync(password,user.password)
          ...
          console.log(user)
        const token= jwt.sign({id:user._id},/*config.get("SECRET_KEY")*/process.env.SECRET_KEY,{expiresIn:"1h"})
            return res.json({
